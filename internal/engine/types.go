...
// Class is the final health classification of a checked link.
type Class string
...
// Ref is one occurrence of a link on a crawled page.
type Ref struct {
...
type Config struct {
	...
	MaxBodyBytes   int64 // cap for verification bodies
	PageBodyBytes  int64 // cap for crawled page bodies
...
// LinkState is the persisted per-link check history used for incremental re-scans.
...
// StateCache is implemented by storage backends; the engine stays storage-agnostic.

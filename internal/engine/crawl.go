...
// checked is the outcome of actually fetching a URL (during crawl or verify).
type checked struct {
	verdict      Verdict
	status       int
	finalURL     string
...
// crawler walks pages on the seed site, verifying each page it visits and
// registering every outgoing link for the verify phase.
type crawler struct {
	e    *Engine
	seed string
	reg  *registry
...
func (c *crawler) enqueue(url string, depth int) bool {
	...
	c.visited[url] = true
	c.pending++
	// Buffer is sized to MaxPages, which the visited gate enforces, so this
	// send never blocks while holding the lock.
...
func (c *crawler) process(ctx context.Context, item crawlItem) {
	...
	c.pages++
	c.mu.Unlock()
	c.e.emitCrawl(c.pages, c.reg.size())
	...
	if item.depth == 0 && fr.Redirected {
		// The seed redirected (e.g. example.com -> www.example.com); treat the
		// landing host as part of the site.
		c.mu.Lock()

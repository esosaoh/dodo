...
// slowLatency is the response-time ceiling above which a success no longer
// grows the host's window.
const slowLatency = 2 * time.Second

// Scheduler applies TCP-style AIMD congestion control per host: each clean,
// fast response grows the host's concurrency window (+1 per window's worth of
// successes), while 429/503/timeouts halve it. This finds each server's real
// capacity instead of guessing a fixed rate limit.
...
// Acquire blocks until the host has capacity (and any Retry-After pause has
// elapsed) or the context is cancelled.
...
func (s *Scheduler) Release(host string, fb Feedback, retryAfter time.Duration) {
	...
	case FeedbackBackoff:
		now := time.Now()
		// Only one multiplicative decrease per short window, so a burst of
		// failures from the same overload event doesn't collapse the limit.
...
// Limit reports the current window for a host (for tests and debugging).
...
// Shutdown wakes all blocked Acquire calls so workers can observe cancellation.
...
// feedbackFor maps a fetch outcome to scheduler feedback.

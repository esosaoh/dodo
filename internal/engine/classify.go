...
// Verdict is the classifier's judgement of a single fetch attempt.
// Confidence expresses how sure we are of the Class, not of the link being broken.
type Verdict struct {
	Class      Class
	Reason     string
	Confidence float64
...
// looksBotProtected detects challenge pages from common WAF/CDN vendors so they
// are reported as "blocked" rather than false-positive "dead".
...
// Repeated identical failures increase confidence; persistent server errors and
// refused connections are treated as dead, persistent rate limiting as blocked.

...
// Fetch GETs with a capped body read. GET rather than HEAD: many servers
// answer HEAD wrong, and soft-404/fragment checks need bodies anyway.
func (f *Fetcher) Fetch(ctx context.Context, rawURL string, opts FetchOpts) *FetchResult {
	...
	if opts.WantBody && (res.IsHTML() || opts.AnyContentType) {
		...
		// drain a little for connection reuse, then bail
